...
type RecordKind = (typeof recordKinds)[number]["key"];
type DataRow = Record<string, any>;
type ModalState =
  | { type: "student"; row?: DataRow }
  | { type: "target"; row?: DataRow }
  | { type: "period"; row?: DataRow }
  | { type: "record"; entity: RecordKind; row: DataRow }
  | null;
type DeleteState = { entity: "student" | "target" | "period" | RecordKind; row: DataRow; label: string } | null;
...
export function SettingsDataCenter({ token }: { token: string }) {
  ...
  const currentRecords = useMemo<DataRow[]>(() => {
    ...
    return rows.filter((row: DataRow) => JSON.stringify(row).toLowerCase().includes(needle));
      ...
      <div className="overflow-hidden rounded-[24px] border border-teal-100 bg-white shadow-soft">
        ...
        <div className="p-4 sm:p-5">
          ...
          {loading ? (
            ...
            <>
              {tab === "students" && <StudentsPanel rows={data.students ?? []} onAdd={() => setModal({ type: "student" })} onEdit={(row: DataRow) => setModal({ type: "student", row })} onDelete={(row: DataRow) => setDeleting({ entity: "student", row, label: row.full_name })} />}
              {tab === "targets" && <TargetsPanel rows={data.classTargets ?? []} onAdd={() => setModal({ type: "target" })} onEdit={(row: DataRow) => setModal({ type: "target", row })} onDelete={(row: DataRow) => setDeleting({ entity: "target", row, label: `${row.class_name} · segmen ${row.segment_no}` })} />}
              {tab === "periods" && <PeriodsPanel rows={data.periods ?? []} onAdd={() => setModal({ type: "period" })} onEdit={(row: DataRow) => setModal({ type: "period", row })} onDelete={(row: DataRow) => setDeleting({ entity: "period", row, label: row.label })} />}
              {tab === "records" && (
                <RecordsPanel
                  kind={recordKind}
                  ...
                  onQuery={setQuery}
                  rows={currentRecords}
                  onEdit={(row: DataRow) => setModal({ type: "record", entity: recordKind, row })}
                  onDelete={(row: DataRow) => setDeleting({ entity: recordKind, row, label: recordLabel(recordKind, row) })}
                />
              )}
...
function StudentsPanel({ rows, onAdd, onEdit, onDelete }: CollectionPanelProps) {
  const [query, setQuery] = useState("");
  const visible = rows.filter((row: DataRow) => `${row.full_name} ${row.class_name} ${row.nis ?? ""}`.toLowerCase().includes(query.toLowerCase()));
  return <div><PanelTitle title="Siswa Binaan" description="Tambah siswa baru, ubah nama/kelas/NIS/status, atau hapus permanen beserta data terkait." onAdd={onAdd} addLabel="Tambah Siswa" /><SearchBox value={query} onChange={setQuery} placeholder="Cari nama atau kelas..." /><div className="mt-4 grid gap-3 md:grid-cols-2">{visible.map((row: DataRow) => <article key={row.id} className="rounded-2xl border border-line bg-white p-4"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-teal-50 text-sm font-extrabold text-teal-800">{row.full_name?.[0] ?? "S"}</div><div className="min-w-0 flex-1"><div className="truncate font-extrabold">{row.full_name}</div><div className="mt-1 text-xs text-muted">Kelas {row.class_name}{row.nis ? ` · NIS ${row.nis}` : ""}</div><div className={cn("mt-2 inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold", row.is_active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500")}>{row.is_active ? "Aktif" : "Nonaktif"}</div></div><RowActions onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></div></article>)}</div>{!visible.length && <Empty text="Tidak ada siswa yang cocok." />}</div>;
}

function TargetsPanel({ rows, onAdd, onEdit, onDelete }: CollectionPanelProps) {
  return <div><PanelTitle title="Target Hafalan per Kelas" description="Kelola rentang target setiap segmen kelas. Perubahan langsung dipakai sebagai acuan dashboard." onAdd={onAdd} addLabel="Tambah Target" /><div className="grid gap-3 md:grid-cols-2">{rows.map((row: DataRow) => <article key={row.id} className="rounded-2xl border border-line p-4"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-cyan-50 text-cyan-700"><Target size={18} /></div><div className="min-w-0 flex-1"><div className="font-extrabold">Kelas {row.class_name}</div><div className="mt-1 text-xs font-semibold text-teal-700">Segmen {row.segment_no}</div><p className="mt-2 text-xs text-muted">{row.start_label} → {row.end_label}</p></div><RowActions onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></div></article>)}</div>{!rows.length && <Empty text="Belum ada target kelas." />}</div>;
}

function PeriodsPanel({ rows, onAdd, onEdit, onDelete }: CollectionPanelProps) {
  return <div><PanelTitle title="Periode Semester" description="Atur sendiri tanggal awal dan akhir semester. Laporan semester nantinya mengikuti periode ini, bukan asumsi bulan tetap." onAdd={onAdd} addLabel="Tambah Semester" /><div className="grid gap-3 md:grid-cols-2">{rows.map((row: DataRow) => <article key={row.id} className={cn("rounded-2xl border p-4", row.is_active ? "border-teal-200 bg-teal-50/40" : "border-line")}><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-teal-700 shadow-sm"><CalendarRange size={18} /></div><div className="min-w-0 flex-1"><div className="font-extrabold">{row.label}</div><div className="mt-1 text-[10px] font-bold uppercase tracking-wide text-muted">{row.code}</div><p className="mt-2 text-xs text-slate-600">{formatDate(row.start_date)} — {formatDate(row.end_date)}</p>{row.is_active && <span className="mt-2 inline-flex rounded-full bg-teal-700 px-2.5 py-1 text-[10px] font-bold text-white">Semester Aktif</span>}</div><RowActions onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></div></article>)}</div>{!rows.length && <Empty text="Belum ada periode semester." />}</div>;
}

function RecordsPanel({ kind, onKind, query, onQuery, rows, onEdit, onDelete }: RecordsPanelProps) {
  return <div><PanelTitle title="Semua Data Aktivitas" description="Cari, koreksi, atau hapus data operasional yang sudah tersimpan. Penambahan aktivitas harian tetap lebih cepat dilakukan dari menu Absensi, Setoran, Murajaah, dan Detail Siswa." /><div className="flex gap-2 overflow-x-auto pb-2">{recordKinds.map((item) => <button key={item.key} type="button" onClick={() => onKind(item.key)} className={cn("shrink-0 rounded-xl px-3 py-2 text-xs font-bold", kind === item.key ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-600")}>{item.label}</button>)}</div><div className="mt-3"><SearchBox value={query} onChange={onQuery} placeholder="Cari nama, kelas, tanggal, surah, catatan..." /></div><div className="mt-4 space-y-2">{rows.slice(0, 150).map((row: DataRow, index: number) => <article key={`${kind}-${row.id ?? row.session_id}-${row.student_id ?? index}`} className="flex flex-col gap-3 rounded-2xl border border-line p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="truncate text-sm font-extrabold">{recordLabel(kind, row)}</div><div className="mt-1 text-xs text-muted">{recordMeta(kind, row)}</div></div><RowActions onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></article>)}</div>{!rows.length && <Empty text="Belum ada data pada kategori ini." />}{rows.length > 150 && <p className="mt-3 text-center text-xs text-muted">Menampilkan 150 data terbaru dari hasil pencarian.</p>}</div>;
}
...
function RecordModal({ token, entity, row, onClose, onSaved }: { token: string; entity: RecordKind; row: DataRow; onClose: () => void; onSaved: () => Promise<void> }) {
