...
export function AttendanceBoard({ students, initial, date }: { students: Student[]; initial: Record<string, AttendanceStatus>; date: string }) {
  ...
  const save = () =>
    startTransition(async () => {
      ...
      setMessage(result.ok ? `Absensi ${formatFriendlyDate(activeDate)} berhasil disimpan.` : result.message ?? "Gagal menyimpan absensi.");
    });
  ...
  async function changeDate(nextDate: string) {
    ...
    if (sessionError) {
      setMessage("Gagal memuat absensi pada tanggal tersebut.");
      setLoadingDate(false);
      return;
    }

    if (!session?.id) {
      setStatuses(Object.fromEntries(students.map((student) => [student.id, "hadir"])));
      setMessage(`Belum ada absensi ${formatFriendlyDate(nextDate)}. Form siap diisi.`);
      setLoadingDate(false);
      return;
    }
    ...
    if (error) {
      ...
      const stored = new Map((attendance ?? []).map((row: any) => [row.student_id, row.status as AttendanceStatus]));
      setStatuses(Object.fromEntries(students.map((student) => [student.id, stored.get(student.id) ?? "hadir"])));
      setMessage(`Absensi ${formatFriendlyDate(nextDate)} dimuat.`);
    }
    setLoadingDate(false);
  }
    ...
    <div className="space-y-4">
      <div className="card flex flex-col gap-3 p-4 lg:flex-row lg:items-end">
        <div className="min-w-0 flex-1">
          ...
          <div className="relative max-w-sm">
            ...
            <input type="date" value={activeDate} onChange={(event) => void changeDate(event.target.value)} className="input pl-10 pr-10" />
            {loadingDate && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-teal-700" size={16} />}
          </div>
          <p className="mt-2 text-[11px] leading-5 text-muted">Bisa memilih tanggal sebelumnya bila absensi baru sempat diinput hari ini. Pergantian tanggal tidak memindahkan halaman.</p>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2">
        <div className="flex gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
          <button className={cn("shrink-0 rounded-xl px-3 py-2 text-xs font-bold", classFilter === "all" ? "bg-teal-700 text-white" : "border border-line bg-white")} onClick={() => setClassFilter("all")}>Semua</button>
          {classes.map((className) => (
            <button key={className} className={cn("shrink-0 rounded-xl px-3 py-2 text-xs font-bold", classFilter === className ? "bg-teal-700 text-white" : "border border-line bg-white")} onClick={() => setClassFilter(className)}>
              Kelas {className}
            </button>
          ))}
        </div>
        <button className="button-secondary w-full sm:ml-auto sm:w-auto" onClick={allPresent}><Users size={16} /> Tandai semua hadir</button>
      </div>
        ...
        {visible.map((student) => (
          <div key={student.id} className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-teal-50 text-sm font-extrabold text-teal-800">{student.full_name.charAt(0)}</div>
              <div className="min-w-0"><div className="truncate text-sm font-bold">{student.full_name}</div><div className="text-xs text-muted">Kelas {student.class_name}</div></div>
            </div>
            <div className="grid grid-cols-2 gap-1 rounded-xl bg-slate-50 p-1 sm:grid-cols-4">
              {options.map((option) => (
                <button key={option.value} data-active={statuses[student.id] === option.value} onClick={() => setStatuses((previous) => ({ ...previous, [student.id]: option.value }))} className={cn("min-h-10 rounded-lg px-2 py-2 text-xs font-bold text-slate-500 transition sm:px-3", option.style)}>
                  {option.label}
                </button>
              ))}
            </div>
      ...
      <div className="flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
        {message ? <p className="text-sm font-semibold text-teal-700">{message}</p> : <p className="text-xs leading-5 text-muted">Tip: tandai semua hadir lalu ubah hanya siswa yang izin, sakit, atau alfa.</p>}
        <button onClick={save} disabled={pending || loadingDate || !students.length} className="button-primary w-full sm:min-w-36 sm:w-auto">
  ...
  async function download() {
    ...
    try {
      ...
      if (sessionIds.length) {
        const { data, error } = await supabase.from("tahfidz_attendance").select("session_id,student_id,status,note").in("session_id", sessionIds);
      ...
      const totalMeetings = selectedSessions.length;
      ...
      const summaryRows = students.map((student) => {
        ...
        return {
          ...
          "Kehadiran (%)": totalMeetings ? Math.round((hadir / totalMeetings) * 100) : 0,
        };
      });
      ...
      for (const month of months) {
        ...
        const rows = students.map((student) => {
          const detail: Record<string, string | number> = { Nama: student.full_name, Kelas: student.class_name };
          let hadir = 0; let izin = 0; let sakit = 0; let alfa = 0;
          ...
          detail.Hadir = hadir; detail.Izin = izin; detail.Sakit = sakit; detail.Alfa = alfa;
          detail["Kehadiran (%)"] = monthSessions.length ? Math.round((hadir / monthSessions.length) * 100) : 0;
          return detail;
        });

        const sheet = XLSX.utils.json_to_sheet(rows);
        sheet["!cols"] = [{ wch: 24 }, { wch: 18 }, ...dates.map(() => ({ wch: 6 })), { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 16 }];
    ...
    <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm md:items-center md:p-5">
      <div className="max-h-[94dvh] w-full max-w-2xl overflow-y-auto rounded-t-[26px] bg-white p-5 shadow-2xl md:rounded-[26px] md:p-6">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" aria-label="Tutup" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-600"><X size={17} /></button>
        </div>

        <div className="mt-5"><label className="mb-2 block text-xs font-bold">Tahun</label><input type="number" min="2020" max="2100" value={year} onChange={(event) => setYear(Number(event.target.value))} className="input max-w-40" /></div>
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between gap-3"><label className="text-xs font-bold">Bulan</label><button type="button" onClick={() => setMonths(months.length === 12 ? [] : Array.from({ length: 12 }, (_, index) => index + 1))} className="text-xs font-bold text-teal-700">{months.length === 12 ? "Kosongkan" : "Pilih semua"}</button></div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
            {monthNames.map((name, index) => {
              const month = index + 1; const active = months.includes(month);
              return <button key={name} type="button" onClick={() => toggleMonth(month)} className={cn("min-h-11 rounded-xl border px-3 py-2 text-xs font-bold transition", active ? "border-teal-700 bg-teal-700 text-white" : "border-line bg-white text-slate-600 hover:border-teal-200 hover:bg-teal-50")}>{name}</button>;
            })}
          </div>
        </div>

        {message && <div className="mt-4 rounded-xl bg-teal-50 px-3 py-2.5 text-xs font-semibold text-teal-800">{message}</div>}
        <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="button-secondary">Batal</button><button type="button" onClick={download} disabled={busy || !months.length} className="button-primary">{busy ? <Loader2 className="animate-spin" size={16} /> : <FileSpreadsheet size={16} />}{busy ? "Menyiapkan..." : "Download Excel"}</button></div>
      </div>
    </div>
  );
}
